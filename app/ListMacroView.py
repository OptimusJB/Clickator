...
class ListMacroView:
    def __init__(self):
        self.charged = False
        self.liste_macros = []  # liste de str (avec extension)
        self.rects_macros = []
    ...
    def charger_macros(self):
        ...
        for element in self.liste_macros:
            if element[len(element)-len(".macro"):] != ".macro":
    ...
    def listen_entry(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for i in range(abs(event.y)):
                if event.y < 0:
                    if self.y + self.y_offset > self.rect_affichage.bottom - 20:
                        self.y_offset -= 30
